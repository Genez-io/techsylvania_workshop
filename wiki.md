...
We are developing an application that generates a list of movies with summarized reviews. We aim to provide a seamless user experience by allowing the user to input his preferences and receive movie suggestions. To give you a better understanding.
...
A prompt that works perfect and doesn't have this problem would be:
    ...
    ${userInput}
    """` 
Now the result will be all the time as a JSON that can be parsed. Another check to avoid prompt injection is to try to parse the result on the server side, and if an error occurs notify the user.
